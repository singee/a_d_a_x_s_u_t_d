		...
		var loadSPE = function(){
			return adax.require('js/SPE.min')
		}
		var loadFountain = function(){
			return adax.require('js/fountain')
		}
		var loadPaths = function(){
			return adax.require('js/paths')
		}
		...
		var init = function()
		{
			...
			.then(function(){
				...
				//map3d.getLayerByName('basemap').material.color.setRGB(0.15, 0.15, 0.15);
			})
		}
		...
		var update = function(){
			...
			if(emitterTicks == undefined)
			{
				fountains.forEach(function(fountain){
					fountain.shadow.tick();
					fountain.particles.tick();
				})
			}
...
var onKeyPressed = function(key, ctrlKey){
	...
	if(key == 'q')
	{
		map3d.getLayerByName('paths').paths.setAnimate(true)
	}
	else if(key == 'w')
	{
		map3d.getLayerByName('paths').paths.setAnimate(false)
	}
...
var fountains = [];
var emitterTicks;
function toggleEmitters()
{
	if(emitterTicks == undefined)
		emitterTicks = 0;
	else
		emitterTicks = undefined;
}
