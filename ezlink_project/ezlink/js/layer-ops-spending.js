...
function updateLayer2(simulatedTime){
	...
	var id = 'STATION_NAME';
	var field = 'TOTAL_AMOUT_SPENT_';

	//queryDataRange(tapOutData, 'timestamp', simulatedTime, simulatedTime+30*60*1000)
	queryData(spendingData, 'timestamp', simulatedTime)
	.then(function(data){
...
function updateLayer(json){
	for(var i=0; i<mrtStationLocations.length; ++i)
	{
		mrtStationLocations[i].totalSpending = 0;
	}
	...
	var data = mrtStationLocations.map(function(record){
		return record.totalSpending/(1500000.0*10);
	})
...
function updateTapoutToLocations(name, value){
	for(var i=0; i<mrtStationLocations.length; ++i)
	{
		...
		{
			mrtStationLocations[i].totalSpending += value;
			break
		}
	}
}
...
function formatInfo(station)
{
	...
	if(mode == 'simulation')
		s += '<p>Total Spending: ' + station.TOTAL_AMOUT_SPENT_ + '</p>';
	else
		s += '<p>Total Spending: ' + station.totalSpending + '</p>';
	s += '</div>';
	s += '</div>';
...
function formatInfo2(station)
{
	...
	if(mode == 'simulation')
		...
		s += '<li>Total Tapouts: ' + station.totalSpending + '</li>';
	s += '</ul>';
