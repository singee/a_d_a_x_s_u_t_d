...
var draggables = [];
function addDraggable(container, handle)
{
	var draggable = Draggable.create(container, {
		type:"x,y",
		edgeResistance:0.65,
		throwProps:true,
		trigger: handle,
	...
	draggables.push(draggable[0]);
}
function enableDrag(flag)
{
	draggables.forEach(function(item){
		var el = document.querySelector(item.vars.trigger);
		/*
		if(flag)
		{*/
			el.style.top = '-224px';
			el.style.bottom = '204px';
...
function setDarkMode(flag)
{
	...
	if(DARKMODE)
	{
		...
		map3d.getLayerByName('grid').visible = false;
		map3d.getLayerByName('basemap').material.color.setRGB(0.25, 0.25, 0.25);
	...
	{
		container.style.backgroundColor = '#A3CCFF';
		map3d.getLayerByName('grid').visible = true;
		map3d.getLayerByName('basemap').material.color.setRGB(0.5, 0.5, 0.5);

		var particles = map3d.getLayerByName('mrt_stations').particles;
		particles.container.material.blending = THREE.NormalBlending;
		var a = particles.getAlphaBuffer();
		for(var i=0; i<a.length; ++i)
		{
			a[i] = 0.5;
